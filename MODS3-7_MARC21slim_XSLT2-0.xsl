		...
		<marc:record xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
			...
			<marc:controlfield tag="008">
				...
				<!-- v3 language -->
					...
					<!-- wij gebruiken rfc3066 [obsolete] die is opgevolgd door RFC5646, de kortste vorm van ISO639 -->
					<xsl:when test="mods:language/mods:languageTerm[@authority='rfc3066']">
						<!-- 2.01 -->
						<xsl:value-of select="mods:language[mods:languageTerm[@authority='rfc3066']][1]/mods:languageTerm[@authority='rfc3066'][1]"/>
					</xsl:when>
			...
			<marc:datafield tag="500" ind1=" " ind2=" ">
				<marc:subfield code="a">Converted from MODS 3.7 to MARCXML using MODS3-7_MARC21slim_XSLT2-0.xsl (Revision Rutger 20250116-001)</marc:subfield>
			</marc:datafield>
...
<!-- Title Info elements -->
	...
	<xsl:template match="mods:titleInfo[@type='uniform'][1]">
		<xsl:choose>
			<!-- v3 role -->
			<xsl:when test="..//mods:role/mods:roleTerm[@type='text']='creator' or mods:name/mods:role/mods:roleTerm[@type='code']='cre'">
				<xsl:call-template name="datafield">
	...
	<xsl:template match="mods:name[@type='personal'][mods:role/mods:roleTerm[@type='text']!='creator' or mods:role/mods:roleTerm[@type='code']!='cre' or not(mods:role)]">
		<xsl:call-template name="datafield">
			...
			<xsl:with-param name="subfields">
				...
				<!-- Toevoeging veld 0 voor studentnummer -->
				<xsl:for-each select="mods:nameIdentifier[@type='studentId']">
					<marc:subfield code="0">studentId:<xsl:value-of select="."/>
					</marc:subfield>
				</xsl:for-each>
				<xsl:for-each select="mods:nameIdentifier[@type='orcid']">
					<marc:subfield code="1">https://orcid.org/<xsl:value-of select="."/></marc:subfield>
					<marc:subfield code="2">ORCID</marc:subfield>
				</xsl:for-each>
				<xsl:for-each select="mods:nameIdentifier[@type='isni']">
					<marc:subfield code="1">https://isni.org/isni/<xsl:value-of select="."/></marc:subfield>
					<marc:subfield code="2">ISNI</marc:subfield>
				</xsl:for-each>
				<xsl:for-each select="mods:nameIdentifier[@type='dai-nl']">
					<marc:subfield code="0"><xsl:value-of select="."/></marc:subfield>
					<marc:subfield code="2">DAI-NL</marc:subfield>
				</xsl:for-each>
				...
				<!-- Indien affiliatie, voeg onze ROR toe -->
				<xsl:if test="mods:affiliation">
					<marc:subfield code="u">https://ror.org/027bh9e22</marc:subfield>
				</xsl:if>
			</xsl:with-param>
		</xsl:call-template>	
	</xsl:template>
	...
	<xsl:template match="mods:name[@type='corporate'][mods:role/mods:roleTerm[@type='text']!='creator' or mods:role/mods:roleTerm[@type='code']!='cre' or not(mods:role)]">
		<xsl:call-template name="datafield">
			...
			<xsl:with-param name="subfields">
				...
				<!-- toevoegen ROR -->
				<xsl:if test="mods:namePart[1]/text()='Leiden University'">
					<marc:subfield code="u">https://ror.org/027bh9e22</marc:subfield>
				</xsl:if>
				
				<!-- toevoeging voor onderdelen van de universiteit -->
				<xsl:for-each select="mods:affiliation">
...
<!-- Language -->	
	...
	<!-- toegevoegd wegens niet werkende taal -->
	<xsl:template match="mods:language">
		<xsl:for-each select="mods:languageTerm[@type='code'][@authority='rfc3066']">
			<xsl:call-template name="datafield">
				<xsl:with-param name="tag">041</xsl:with-param>
				<xsl:with-param name="ind1">0</xsl:with-param>
				<xsl:with-param name="subfields">
	...
	<!-- v3 language -->
	<!-- veld lijkt niet te functioneren 
	<xsl:template match="mods:language/mods:languageTerm[@authority='rfc3066']">
		<marc:datafield tag="041" ind1="werkt" ind2="niet" />
		<xsl:call-template name="datafield">
			<xsl:with-param name="tag">041</xsl:with-param>
			<xsl:with-param name="ind1">0</xsl:with-param>
			<xsl:with-param name="ind2">7</xsl:with-param>
...
<!-- Note -->	
	<!-- 1/04 fix -->
	<!-- verwijderd plagiarismCheck -->
	<xsl:template match="mods:note[not(@type='statement of responsibility'or @type='plagiarismCheck' or @type='coverageCourt' or @type= 'annotationNumber' or @type='researchArea' or 'sponsorship')]">
		<xsl:call-template name="datafield">
			<xsl:with-param name="tag">
				<xsl:choose>
					...
					<!-- hebben we nog vrije velden voor notes, ects heeft een nodig -->
					<xsl:when test="@type='ectsCredits'">599</xsl:when>
	...
	<!-- toevoeging sponsorship / hier kan ook nwo -->
	<xsl:template match="mods:note[@type='sponsorship']">
		<xsl:call-template name="datafield">
	...
	<!-- toevoeging Coverage Court -->
	<xsl:template match="mods:note[@type='coverageCourt']">
		<xsl:call-template name="datafield">
			<xsl:with-param name="tag">710</xsl:with-param>
			<xsl:with-param name="subfields">
				<marc:subfield code="a"><xsl:value-of select="."/></marc:subfield>
				<marc:subfield code="i">Coverage Court</marc:subfield>
			</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
	
	<!-- toevoegen annotationNumber als identifier -->
	<xsl:template match="mods:note[@type='annotationNumber']">
		<xsl:call-template name="datafield">
			<xsl:with-param name="tag">024</xsl:with-param>
			<xsl:with-param name="ind1">8</xsl:with-param>
			<xsl:with-param name="subfields">
				<marc:subfield code="a"><xsl:value-of select="."/></marc:subfield>
				<marc:subfield code="q">Annotation Number</marc:subfield>
			</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
	
	<!-- toevoegen case number, dat is gerelateerd, maar niet het object zelf -->
	<xsl:template match="mods:part">
		<xsl:choose>
			<xsl:when test="mods:detail[@type='caseNumber']">
				<xsl:call-template name="datafield">
					<xsl:with-param name="tag">024</xsl:with-param>
					<xsl:with-param name="ind1">8</xsl:with-param>
					<xsl:with-param name="subfields">
						<marc:subfield code="a"><xsl:value-of select="mods:detail[@type='caseNumber']/mods:number/text()"/></marc:subfield>
						<marc:subfield code="d"><xsl:value-of select="..//mods:date/text()"/></marc:subfield>
						<marc:subfield code="q">Case Number</marc:subfield>
					</xsl:with-param>
				</xsl:call-template>
			</xsl:when>
		</xsl:choose>
	...
	<!-- toevoegen researchArea -->
	<xsl:template match="mods:note[@type='researchArea']">
		<xsl:call-template name="datafield">
			...
			<xsl:with-param name="subfields">
				<marc:subfield code="a"><xsl:value-of select="."/></marc:subfield>
				<marc:subfield code="2">Research Area</marc:subfield>
			</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
...
-->
	...
	<xsl:template match="mods:subject[local-name(*[1])='topic']">
		<xsl:call-template name="datafield">
			<xsl:with-param name="tag">650</xsl:with-param>
			<!-- we maken geen gebruik van levels in de subject ind1 moet 0 zijn. -->
			<xsl:with-param name="ind1">0</xsl:with-param>
			<xsl:with-param name="subfields">
				<marc:subfield code="a">
...
<!-- Identifiers -->	
	...
	<!-- toegevoegd de converis identifiers -->
	<xsl:template match="mods:identifier[@type='doi'] | mods:identifier[@type='hdl'] | mods:identifier[@type='lucris-id'] | mods:identifier[@type='lumc-id']">
		<xsl:call-template name="datafield">
			<xsl:with-param name="tag">024</xsl:with-param>
			<xsl:with-param name="subfields">
				<marc:subfield code="a">
	...
	<!-- toegevoegd eisbn -->
	<xsl:template match="mods:identifier[@type='eisbn']">
		<xsl:call-template name="datafield">
			<xsl:with-param name="tag">020</xsl:with-param>
			<xsl:with-param name="subfields">
				<marc:subfield>
				...
				<marc:subfield code="q">(eISBN)</marc:subfield>
			</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
...
<!-- v3.4 add physical location url 
	...
	<!-- toegevoegd voor book reviews reviewOf -->
	<xsl:template match="mods:relatedItem[@type='reviewOf']">	
		<xsl:call-template name="datafield">
			...
			<xsl:with-param name="subfields">
				...
				<!-- toevoeging voor betere splitsing onderdelen in veld g -->
				<xsl:for-each select="mods:part/mods:detail[@type='volume']">
					<marc:subfield code="g">volume:<xsl:value-of select="mods:number/text()"/></marc:subfield>
				</xsl:for-each>
				<xsl:for-each select="mods:part/mods:detail[@type='issue']">
					<marc:subfield code="g">issue:<xsl:value-of select="mods:number/text()"/></marc:subfield>
				</xsl:for-each>
				<xsl:for-each select="mods:part/mods:extent[@unit='page']/mods:start">
					<marc:subfield code="g">start:<xsl:value-of select="."/></marc:subfield>
				</xsl:for-each>
				<xsl:for-each select="mods:part/mods:extent[@unit='page']/mods:end">
					<marc:subfield code="g">end:<xsl:value-of select="."/></marc:subfield>
				</xsl:for-each>
				<marc:subfield code="i">Review of</marc:subfield>
	...
	<xsl:template match="mods:relatedItem[@type='host']">
		<xsl:call-template name="datafield">
			...
			<xsl:with-param name="subfields">
				...
				<!-- toegevoegd originInfo -->
				<xsl:if test="mods:originInfo">
					<marc:subfield code="d">
						<xsl:if test="mods:originInfo/mods:place/mods:placeTerm"><xsl:value-of select="mods:originInfo/mods:place/mods:placeTerm/text()"/></xsl:if>
						<xsl:if test="(mods:originInfo/mods:place/mods:placeTerm and mods:originInfo/mods:publisher)">, </xsl:if>
						<xsl:if test="mods:originInfo/mods:publisher"><xsl:value-of select="mods:originInfo/mods:publisher/text()"/></xsl:if>
					</marc:subfield>
				</xsl:if>
		...
		<!-- toegevoegd conferentie -->
		<xsl:if test="mods:name[@type='conference']">
			<xsl:call-template name="datafield">
				<xsl:with-param name="tag">711</xsl:with-param>
				<xsl:with-param name="ind1">2</xsl:with-param>
				<xsl:with-param name="subfields">
					<xsl:if test="mods:name/mods:namePart[not(@type='date')]">
						<marc:subfield code="a"><xsl:value-of select="mods:name/mods:namePart[not(@type='date')]/text()"/></marc:subfield>
					</xsl:if>
					<xsl:if test="mods:name/mods:description">
						<marc:subfield code="c"><xsl:value-of select="mods:name/mods:description/text()"/></marc:subfield>
					</xsl:if>
					<xsl:for-each select="mods:name/mods:namePart[@type='date']">
						<marc:subfield code="d"><xsl:value-of select="."/></marc:subfield>
					</xsl:for-each>
				</xsl:with-param>
			</xsl:call-template>
		</xsl:if>
...
-->
	<xsl:template name="relatedItem76X-78X">
		...
		<xsl:for-each select="mods:note">
			<!-- Advanced publication apart -->
			
			<xsl:choose>
				<xsl:when test="@type='advancedPublication'">
					<xsl:choose>					
						<xsl:when test=".='Yes'">
							<marc:subfield code="n">Advanced Publication</marc:subfield>
						</xsl:when>
					</xsl:choose>
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:if test="mods:name">
			<marc:subfield code="a">
				<xsl:variable name="nameString">
					<xsl:for-each select="mods:name">
						<!-- aangepast voor volledige naam !-->
						<xsl:choose>
							<xsl:when test="mods:namePart[@type='family']">
								<xsl:if test="mods:namePart[@type='family']"><xsl:value-of select="mods:namePart[@type='family']/text()"/></xsl:if>
								<xsl:if test="mods:namePart[@type='given']">, <xsl:value-of select="mods:namePart[@type='given']/text()"/></xsl:if>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="mods:namePart[1][not(@type='date')]"/>
								<xsl:if test="mods:namePart[2][@type='date']">
									<xsl:value-of select="concat(' ',mods:namePart[2][@type='date'])"/>
								</xsl:if>
							</xsl:otherwise>
						</xsl:choose>
						<!-- 2.01 -->
